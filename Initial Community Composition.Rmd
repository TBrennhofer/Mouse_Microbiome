...
as.data.frame(initial_abun_tax) %>%
  ...
  ggplot(aes(x = Mouse, y = abun, fill = Phylum)) +
    ...
    theme(
      ...
      strip.text = element_text(size = 16, face = "bold", color = "midnightblue"),
      legend.key = element_rect(fill = "white", colour = "midnightblue"),
      axis.text=element_text(size=18, face = "bold", color = "midnightblue"),
      axis.title=element_text(size=20, face = "bold", color = "midnightblue"),
      legend.title = element_text(face = "bold", size = 16, color = "midnightblue"),
      legend.text = element_text(size = 14, color = "midnightblue")) +
...
p <- j %>%
  ...
  theme(
      ...
      legend.justification = c("right", "top"),
      legend.box.just = "right",
      axis.text=element_text(size=16, face = "bold", colour = "midnightblue"),
      axis.title=element_text(size=20, face = "bold", colour = "midnightblue"),
      legend.title = element_text(face = "bold", size = 16, colour = "midnightblue"),
      legend.text = element_text(size = 14, colour = "midnightblue"))
```
...
#Graph results  
p <- ggplot(melted_bc_matrix, aes(x = Hour, y = BC, group = Abx, color = Abx)) +
  scale_color_manual(
    values = c("midnightblue", "red"),
    name="Antibiotic\nTreated",
    breaks=c("N", "Y"),
  ...
  theme(
    legend.key = element_rect(fill = "white", colour = "midnightblue"),
    legend.box.just = "right",
    axis.text=element_text(size=20, face = "bold", color = "midnightblue"),
    axis.title=element_text(size=24, face = "bold", color = "midnightblue"),
    legend.title = element_text(face = "bold", color = "midnightblue", size = 16),
...
tmb <- ggplot(rp_abun_tax_trait, aes(x = pos_tval, y = neg_tval, color = Abx, shape = Entero_Sort)) +
  geom_jitter(width = .25, height = .5, size = 3) +
  scale_color_manual(
  ...
  theme(
    axis.text=element_text(size=24, color = "midnightblue", face = "bold"),
    axis.title=element_text(size=28, face = "bold", color = "midnightblue"),
    legend.position = c(.98, .98),
    legend.justification = c("right", "top"),
    legend.box.just = "center",
    legend.title = element_text(size = 20, face = "bold", color = "midnightblue"),
    legend.text = element_text(size = 16, color = "midnightblue"),
...
wl_abun_tax_trait <- wl_abun_tax_trait_raw %>%
  ...
  summarise(
    pos_tval = sum(tval > 0), 
    neg_tval = sum(tval < 0)) %>%
  filter(pos_tval + neg_tval > 1 & Abx == "Y" & Trait %in% c("Forms_biofilms", "Gene_number", "Gram_positive", "Motility", "Obligate_anaerobe", "Sporulation")) %>%
mutate(Status = if_else(neg_tval == 0, "Winner", if_else(pos_tval == 0, "Loser", "Differential Responder")))

p <- ggplot(wl_abun_tax_trait, aes(x = Status, y = Value, group = Status, color = Status)) +
  geom_boxplot(size = 1.25) +
  geom_jitter(width = 0.25, height = .25, size = 1.5) +
  scale_x_discrete(limits = c("Loser","Differential Responder","Winner")) +
  theme_light() +
  labs(x = "Status Classification", y = "OTU Trait Value") +
  scale_color_manual(
    values = c("purple4", "red", "cornflowerblue"), 
    breaks=c("Loser","Differential Responder","Winner")) +
...
entero_data <- abun_tax %>%
  mutate(Entero_Sort = if_else(Family == "Enterobacteriaceae", "Y", "N")) %>%
  filter(Hour %in% c(0,120) & Entero_Sort == "Y" & abun > 0)%>%
  select(Hour, Mouse, otu, abun) %>%
  group_by(Mouse, otu) %>%
  ...
  group_by(Mouse, Hour) %>%
  summarise(Entero_Abun = sum(abun)) %>%
  spread(Hour, Entero_Abun) %>%
  rename(Hour120 = "120", Hour0 = "0")

entero_data <- abun_tax %>%
  filter(Family == 'Enterobacteriaceae' & Hour %in% c(0,120)) %>%
  group_by(Mouse, Hour) %>%
  summarise(abun = sum(abun)) %>%
  spread(Hour, abun, fill = 0)
  
entero_data_rich <- abun_tax %>%
  filter(Family == 'Enterobacteriaceae' & Hour %in% c(0,120) & abun > 0) %>%
...
entero_data_diversity <- abun_tax %>%
  filter(Hour %in% c(0,120) & abun > 0 & Family == "Enterobacteriaceae") %>%
  group_by(Mouse) %>%
...
community_data <- abun_tax %>%
  mutate(Entero_Sort = if_else(Family == "Enterobacteriaceae", "Y", "N")) %>%
  filter(Hour %in% c(0,120) & abun > 0) %>%
  group_by(Mouse) %>%
  mutate(total_rich = n_distinct(otu)) %>%
  group_by(Mouse, Hour) %>%
  summarise(rich = n_distinct(otu), total_rich = unique(total_rich), H = (-((rich/total_rich)*log(rich / total_rich)))) %>%
  group_by(Mouse) %>%
  summarise(delta_rich = diff(rich), delta_H = diff(H))

all_data <- full_join(community_data, entero_data_rich, by = "Mouse")
all_data <- full_join(all_data, entero_data_diversity, by = "Mouse")
  #select(-"rich", -"total_rich")
 
ggplot(all_data, aes(x = Entero_Diversity_Hour_0, y = -(delta_H), color = Abx)) +
  geom_point()+
  stat_smooth(method = "lm")

ggplot(all_data, aes(x = Entero_Diversity_Hour_120, y = -(delta_H), color = Abx)) +
  geom_point()+
  stat_smooth(method = "lm")

ggplot(all_data, aes(x = Entero_Rich_Hour_0, y = -(delta_H), color = Abx)) +
  geom_point()+
  stat_smooth(method = "lm")

ggplot(all_data, aes(x = Entero_Rich_Hour_0, y = delta_rich, color = Abx)) +
  geom_point()+
  stat_smooth(method = "lm")
...
tval_iso_clean <- tval_iso %>%
  ...
  mutate(
    pos_tval = sum(tval > 0), 
    neg_tval = sum(tval < 0)) %>%
  filter(pos_tval + neg_tval > 1 & pos_tval >=1 & neg_tval >=1 & Abx == "Y") %>%
  ungroup() %>%
